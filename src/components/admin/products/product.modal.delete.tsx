...
import { apiDeleteProduct } from "../../../config/api";
import { IProduct } from "../../../types/backend";
import CustomToast from "../../common/toast.message";

interface IProductModalDeleteProps {
...
const ProductModalDelete = (props: IProductModalDeleteProps) => {
  const { isOpenDeleteModal, dataInit, setDataInit, onClose, reloadTable } = props;
  const handleDelete = async () => {
    const res = await apiDeleteProduct(dataInit?.id ?? "");
  ...
  return (
    ...
    >
        {isOpenDeleteModal && (
            <div className="z-[-1] transition duration fixed inset-0 bg-gray-900/50">
            </div>
        )}
      <div className="sm:max-w-lg sm:w-full m-3 sm:mx-auto">
        <div className="flex flex-col bg-white border border-gray-200 shadow-2xs rounded-xl pointer-events-auto">
          <div className="flex justify-between items-center py-3 px-4 border-b border-gray-200">
            ...
            >
              Bạn có chắc chắn muốn xóa sản phẩm: {dataInit?.name}?
            </h3>
            <button
              type="button"
          ...
          <div className="p-4 overflow-y-auto">
            <p className="mt-1 text-gray-800 text-sm">
              Hành động này không thể hoàn tác. Sản phẩm này sẽ bị xóa vĩnh
              viễn khỏi hệ thống.
            </p>
          </div>
