...
const SupplierModalDelete = (props: ISupplierModalDeleteProps) => {
  const { isOpenDeleteModal, dataInit, setDataInit, onClose, reloadTable } =
    props;
  const handleDelete = async () => {
    const res = await apiDeleteSupplier(dataInit?.id ?? "");
    if (res?.data?.statusCode === 200) {
      toast.success(<CustomToast message="Xóa nhà cung cấp thành công!" />);
    } else {
      toast.error(<CustomToast message="Xóa nhà cung cấp thất bại!" />);
    }
    onClose();
  };
      ...
      <div className="sm:max-w-lg sm:w-full m-3 sm:mx-auto">
        <div className="flex flex-col bg-white border border-gray-200 shadow-2xs rounded-xl pointer-events-auto">
          <div className="flex justify-between items-center py-3 px-4 border-b border-gray-200">
            ...
            >
              Bạn có chắc chắn muốn xóa nhà cung cấp: {dataInit?.name}?
            </h3>
            <button
              type="button"
          ...
          <div className="p-4 overflow-y-auto">
            <p className="mt-1 text-gray-800 text-sm">
              Hành động này không thể hoàn tác. Nhà cung cấp này sẽ bị xóa vĩnh
              viễn khỏi hệ thống.
            </p>
          </div>
